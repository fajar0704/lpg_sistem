...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-slate-500 text-sm mt-1">Rincian identitas dan riwayat transaksi pembelian tabung gas LPG.</p>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('sub-pangkalan.customers.edit', $customer) }}" class="inline-flex items-center justify-center gap-2 bg-amber-50 hover:bg-amber-100 text-amber-700 font-bold px-4 py-2.5 rounded-xl border border-amber-200/50 shadow-sm transition">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <!-- Left: Customer Profile & Photo KTP -->
        <div class="lg:col-span-1 space-y-6">
            <!-- Profil Card -->
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-5">
                <div class="border-b border-slate-100 pb-4">
                    <span class="inline-flex items-center px-2.5 py-1 bg-blue-50 text-blue-700 border border-blue-100/50 rounded-full text-xs font-semibold uppercase tracking-wider mb-2">
                        🏠 {{ $customer->category_label }}
                    </span>
                    <h3 class="text-xl font-extrabold text-slate-800">{{ $customer->name }}</h3>
                    <p class="text-xs text-slate-400 font-medium mt-0.5">Terdaftar sejak {{ $customer->created_at->translatedFormat('d F Y') }}</p>
                </div>

                <div class="space-y-4">
                    <div>
                        <span class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">NIK (KTP)</span>
                        <span class="font-mono text-base font-bold text-slate-700">{{ $customer->ktp }}</span>
                    </div>

                    <div>
                        <span class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Nomor Telepon</span>
                        <span class="text-slate-600 font-semibold">{{ $customer->phone ?? 'Tidak ada nomor telepon' }}</span>
                    </div>

                    <div>
                        <span class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Alamat Domisili</span>
                        <span class="text-slate-600 text-sm font-semibold block mt-0.5 leading-relaxed">{{ $customer->address ?? 'Tidak ada alamat' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-4">
                <h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider">Dokumentasi Foto KTP (Klik untuk memperbesar)</h4>
                @if($customer->photo)
            ...
            <!-- Dokumentasi Foto KK -->
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-4">
                <h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider">Dokumentasi Foto KK (Klik untuk memperbesar)</h4>
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                    <h3 class="font-bold text-slate-800 text-lg">Riwayat Pembelian Tabung</h3>
                        ...
                        <tbody class="divide-y divide-slate-100 font-medium text-slate-700">
                            ...
                            <tr class="hover:bg-slate-50/30 transition">
                                ...
                                <td class="px-6 py-4 font-bold text-slate-800">
                                    {{ $sale->transaction_date->translatedFormat('d F Y') }}
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <span class="inline-flex items-center px-2.5 py-1 bg-indigo-50 text-indigo-700 border border-indigo-100/50 rounded-full text-xs font-bold">
                                        {{ $sale->tabung_type }}
                                    </span>
                                </td>
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center text-slate-400 font-semibold">
                                    Belum ada catatan pembelian untuk pelanggan ini.
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
